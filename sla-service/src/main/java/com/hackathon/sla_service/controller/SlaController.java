...
public class SlaController {
    ...
    public ResponseEntity<SlaSummaryResponse> getB2cSummary(
            ...
            @RequestParam(value = "manager_id", required = false)
            String managerId,

            @RequestParam(value = "qualification", required = false)
            String qualification
    ) {
        return ResponseEntity.ok(
                slaSummaryService.getB2cSummary(dateFrom, dateTo, managerId, qualification)
        );
    }
}